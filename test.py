import os
import pandas as pd
from tkinter import Tk, filedialog, Label, Button, Entry, messagebox  # Agrega Entry a los imports
...
def process_files():
    ...
    try:
        ...
        date_str = date_entry.get()  # El parámetro 'date' puede ser opcional
        if date_str:
        ...
        if not all(column in excel_data.columns for column in required_columns):
            messagebox.showerror("Error", "El archivo Excel no contiene las columnas requeridas: 'Agent', 'Customer ID', 'Account Name'")
            return
